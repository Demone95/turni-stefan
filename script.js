const REF=new Date(2026,6,6),cycle=[2,1,3];let baseShift=Number(localStorage.getItem('baseShift'))||0;
...
document.querySelectorAll('[data-shift]').forEach(b=>b.onclick=()=>{baseShift=Number(b.dataset.shift);localStorage.setItem('baseShift',baseShift);setup.classList.add('hidden');show(new Date())});
